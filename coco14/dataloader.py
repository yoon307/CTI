...
class COCO14ImageDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # img = np.asarray(imageio.imread(get_img_path(name_str, self.coco14_root))) #original
        img = np.array(PIL.Image.open(get_img_path(name_str,self.coco14_root)).convert("RGB")) #added
        # gray scale
        if len(img.shape) == 2:
...
class COCO14ClassificationDatasetMSF(COCO14ClassificationDataset):
    ...
    def __getitem__(self, idx):
        ...
        # img = imageio.imread(get_img_path(name_str, self.coco14_root)) #original
        img = np.array(PIL.Image.open(get_img_path(name_str,self.coco14_root)).convert("RGB")) #added
